...
public:
	...
	template <class... Args>
	explicit scene_renderer_t(const scene_t& scene, const Args&... args);

	void render(const scene_view_t& view, const glpp::render::camera_t& camera);
...
template<class ShadingModel>
template <class... Args>
scene_renderer_t<ShadingModel>::scene_renderer_t(const scene_t& scene, const Args&... args)
{
	std::transform(
		...
		[&](const material_t& material) {
			return mesh_renderer_t<ShadingModel>{ material, args... };
		}
	);
...
template<class ShadingModel>
void scene_renderer_t<ShadingModel>::render(const scene_view_t& view, const glpp::render::camera_t& camera) {
