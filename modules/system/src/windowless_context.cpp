...
windowless_context_t::windowless_context_t(windowless_context_t&& mov) :
   display(mov.display),
   config(mov.config),
   context(mov.context),
   surface(mov.surface),
   num_config(mov.num_config)
{
    mov.display = nullptr;
    mov.config = nullptr;
    mov.context = nullptr;
...
windowless_context_t& windowless_context_t:: operator=(windowless_context_t&& mov)
{
    display = mov.display;
    config = mov.config;
    context = mov.context;
...
windowless_context_t::windowless_context_t(const driver_t driver) {
...
windowless_context_t::~windowless_context_t() {
    if(display != nullptr) {
        if(context != nullptr) {
